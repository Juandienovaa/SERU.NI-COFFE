  ...
  return (
    <section className="relative w-full h-[100dvh] flex flex-col md:flex-row overflow-hidden bg-white">
      ...
      <div className="absolute inset-0 flex flex-col md:flex-row z-0 pointer-events-none">
        {/* Left: White */}
        <div className="w-full md:w-[55%] h-[60%] md:h-full bg-white"></div>
        {/* Right: Orange */}
        <div className="w-full md:w-[45%] h-[40%] md:h-full bg-orange-500 rounded-t-[32px] md:rounded-t-none md:rounded-l-[40px]"></div>
      </div>
      ...
      <div className="relative z-10 w-full h-full flex flex-col md:flex-row container mx-auto px-6 md:px-12 lg:px-24">
        ...
        >
          ...
          <div className="flex flex-row items-center gap-3 md:gap-4 w-full sm:w-auto">
            <Link
              href="/menu-online"
              className="group flex items-center justify-center gap-2 px-6 md:px-8 py-3 md:py-4 bg-neutral-900 text-white rounded-full font-semibold text-sm md:text-lg transition-all hover:bg-black hover:scale-105 active:scale-95 flex-1 sm:flex-none shadow-xl"
            >
            ...
            <button 
              ...
              className="flex items-center justify-center gap-2 px-6 md:px-8 py-3 md:py-4 bg-transparent border-2 border-neutral-200 text-neutral-900 rounded-full font-semibold text-sm md:text-lg transition-all hover:border-orange-500 hover:text-orange-500 flex-1 sm:flex-none"
            >
              Lihat Menu
        ...
        <motion.div 
          ...
          className="w-full md:w-[45%] h-[40%] md:h-full flex flex-col justify-center items-end text-white px-6 md:pr-20 pb-8 md:pb-0 relative"
        >
          <div className="flex flex-col items-end gap-1 mb-6 md:mb-12">
            <span className="text-xs md:text-sm font-medium text-orange-200 uppercase tracking-widest">Start From</span>
            <span className="text-3xl md:text-5xl font-black drop-shadow-md">Rp 15.000</span>
          </div>
          ...
          <div className="flex relative mt-4">
            ...
            <div className="flex flex-col gap-6 text-right pr-8">
              {[
                ...
                <span 
                  key={i} 
                  className={`text-sm md:text-lg font-medium transition-colors ${item.active ? 'text-white' : 'text-orange-200 hover:text-white cursor-pointer'}`}
                >
                  {item.name}
                </span>
              ))}
